...
def main():
    ...
    imgOriginalScene  = cv2.imread("LicPlateImages/20.jpg")               # open image
...
def drawRedRectangleAroundPlate(imgOriginalScene, licPlate):
    ...
    cv2.line(imgOriginalScene, (int(p2fRectPoints[0][0]), int(p2fRectPoints[0][1])), (int(p2fRectPoints[1][0]), int(p2fRectPoints[1][1])), SCALAR_RED, 2)         # draw 4 red lines
...
def writeLicensePlateCharsOnImage(imgOriginalScene, licPlate):
    ...
    csv_file = csv.reader(open('iso.csv', "r"), delimiter=",")
    for row in csv_file:
        if licPlate.strChars[:2] == row[0]:
            print (row)
            state=row[1]
    l=[[licPlate.strChars,state,time.asctime( time.localtime(time.time()))]]
    print(l)
